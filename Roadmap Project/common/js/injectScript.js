...
if (window.location.href.indexOf('WopiFrame.aspx') > -1) {
...
InjectScript.fn.getLinks = function () {
    InjectScript.vars.allLinks = [];
    var sessionKeyNavLinksCat = "ITReportingnavLinksCat";
...
InjectScript.fn.showHideNavEventHndler = function () {
    $(".shownav").click(function () {
        if ($("#suiteBarDelta").css("display") === "block") {
            ...
            $("#s4-workspace").css("top", '70px');
        } else {
            $('#suiteBarDelta').show();
            $("#s4-ribbonrow").show();
            $("#suiteBarDelta").css("margin-top", "65px");
            $("#s4-workspace").css("top", '150px');
        }
    });
}
...
InjectScript.fn.addPadding = function () {
    ...
    // }
    $("#s4-workspace").css({ "top": (($('#suiteBarDelta').height() - 30) + $("#globalNavBox").height() + 15) + "px", "position": "fixed", "z-index": "1" });
};
...
InjectScript.fn.InjectScriptToMaster = function () {
    ...
    var TOP_NAV = '<div class="header"><nav class="navbar navbar-expand-md navbar-dark bg-dark fixed-top"><div class="d-flex w-50 order-0 p-2">' +
        '<a class="navbar-brand mr-1" href="#"><img onclick="InjectScript.fn.goToHome();" src="/sites/IT%20Reporting/Resources/common/img/BT_colour_mark.svg" style="height:3.2vw"class="d-inline-block align-middle" alt="" data-themekey="#">' +
        '<span class="d-inline-block logo-txt" onclick="InjectScript.fn.goToHome();">IT Reporting</span></a></div><div style="display:none !important;" class="navbar-collapse collapse justify-content-center order-2" id="collapsingNavbar">' +
        ...
        '<div class="navbar-collapse w-50 order-1 order-md-last otherlinks" style="margin-right: 30px;"><div class="topRibbon-show-hide shownav" onclick="InjectScript.fn.openTopRibbon();"><i class="fa fa-gear"></i></div><ul class="navbar-nav ml-auto horz">' + subLinks + '</ul>' +
...
/**************************************************************************/
/// Function name: getFieldsFromContentType
/// Summary:  To get fields from content type by REST using
/**************************************************************************/
InjectScript.fn.getFieldsFromContentType = function (url, contentTypeId, sync) {
    var dfd = $.Deferred();
    $.ajax({
        url: url + "/_api/web/contenttypes('" + contentTypeId + "')/fields",
        async: !sync,
        method: "GET",
        headers: { "Accept": "application/json; odata=verbose" },
        success: function (data) {
